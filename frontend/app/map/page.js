...
export default function MapPage() {
  ...
  const fetchData = useCallback(async (type = 'all') => {
    ...
    try {
      ...
      setFetchError('Could not load incident data. Please refresh.');
    } finally {
  ...
  return (
    <div className="h-screen flex flex-col bg-gray-50 overflow-hidden">
      ...
      <div className="flex-1 flex overflow-hidden">
        ...
        <aside className="flex-none bg-white border-r border-gray-100 flex flex-col overflow-y-auto z-10" style={{ width: '272px' }}>
          ...
          <div className="p-4">
            ...
            <button onClick={() => fetchData(filter)} className="mt-3 text-xs text-pink-500 hover:text-pink-700 underline">
              Refresh data
            </button>
          </div>

          {fetchError && (
            <div className="mx-3 mb-3 bg-red-50 border border-red-200 rounded-xl p-3">
          ...
          {selectedIncident && (
            <div className="mx-3 mb-4 bg-indigo-50 border border-indigo-100 rounded-xl p-4">
              <div className="flex items-start justify-between mb-2">
                ...
                <button onClick={() => setSelectedIncident(null)} className="text-gray-300 hover:text-gray-500 text-xl leading-none">×</button>
              </div>
              {selectedIncident.date && (
                <p className="text-xs text-indigo-400 mt-2">
                  {new Date(selectedIncident.date).toLocaleDateString('en-GB', { day: 'numeric', month: 'long', year: 'numeric' })}
                </p>
              )}
              <span className={`inline-block mt-2 text-xs px-2 py-0.5 rounded-full font-medium ${
                ...
                {((selectedIncident.severity || 'medium').charAt(0).toUpperCase() + (selectedIncident.severity || 'medium').slice(1))} severity
              </span>
        ...
        <div className="flex-1 relative">
          ...
          <div className="absolute bottom-6 left-1/2 -translate-x-1/2 z-20 bg-white/90 backdrop-blur-sm shadow rounded-full px-4 py-1.5 whitespace-nowrap">
            <p className="text-xs text-gray-400">🔒 Approximate locations · No personal data · Registered users only</p>
          </div>
        </div>
      </div>
    </div>
