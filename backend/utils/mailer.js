...
const sendEmail = async ({ to, toName, subject, html }) => {
  ...
  email.sender = { name: 'SafeHer', email: 'shompaakterj@gmail.com' };
...
<table width="520" cellpadding="0" cellspacing="0" style="background:#fff;border-radius:16px;overflow:hidden;border:2px solid #fca5a5">
  ...
  <tr><td style="padding:32px">
    ...
    <tr><td style="background:#fef2f2;border:1px solid #fca5a5;border-radius:10px;padding:20px">
      ...
      ${mapLink ? `<a href="${mapLink}" style="display:inline-block;background:#dc2626;color:#fff;padding:12px 28px;border-radius:999px;text-decoration:none;font-size:14px;font-weight:700">📍 Open in Google Maps →</a>` : ''}
    </td></tr></table>
    <table width="100%" cellpadding="0" cellspacing="0" style="margin-bottom:16px">
    <tr><td style="background:#fff7ed;border:1px solid #fed7aa;border-radius:10px;padding:18px">
      ...
      ${senderPhone ? `<p style="margin:0;font-size:14px;color:#374151"><b>Phone:</b> <a href="tel:${senderPhone}" style="color:#dc2626;text-decoration:none;font-weight:700">${senderPhone}</a></p>` : ''}
    </td></tr></table>
    ...
    <tr><td style="background:#f0fdf4;border:1px solid #86efac;border-radius:10px;padding:20px">
      ...
      <p style="margin:0;font-size:14px;color:#374151">3️⃣ Call emergency services: <b style="color:#dc2626;font-size:18px"> 999</b></p>
    </td></tr></table>
  ...
  console.log(`✅ SOS email → ${toEmail} (${toName})`);
};

const sendNewReportEmail = async ({ reportCode, incidentType, location, isAnonymous, contactName }) => {
  const adminEmail = process.env.TEAM_EMAIL || process.env.MAIL_USER || 'shompaakterj@gmail.com';
  await sendEmail({
    to: adminEmail,
    toName: 'SafeHer Admin',
...
<table width="480" cellpadding="0" cellspacing="0" style="background:#fff;border-radius:16px;overflow:hidden;border:1px solid #f9a8d4">
  ...
  console.log(`✅ Report notification → ${adminEmail} | Code: ${reportCode}`);
};
